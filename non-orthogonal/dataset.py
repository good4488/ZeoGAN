...
class EnergyShapeDataset:
    ...
    def _read_and_nomalize_cell(self, path):
        ...
        a, b, c = data["CELL_PARAMETERS"]
        alpha, beta, gamma = data["CELL_ANGLES"]

        aa = a*a
        bb = b*b
        cc = c*c
        bc = b*c * np.cos(alpha / 180.0 * np.pi)
        ca = c*a * np.cos(beta / 180.0 * np.pi)
        ab = a*b * np.cos(gamma / 180.0 * np.pi)

        cell = np.array([aa, bb, cc, bc, ca, ab])
        # Normalization. Neglecting cmin.
        cell /= cmax**2
    ...
    def write_visit_sample(self, *, cell, grid, stem, save_dir="."):
        ...
        cmin, cmax = self.cell_length_scale
        cell = cell * cmax**2
        aa, bb, cc, bc, ca, ab = cell

        a = np.sqrt(aa)
        b = np.sqrt(bb)
        c = np.sqrt(cc)
        alpha = np.arccos(bc / b / c) / np.pi * 180.0
        beta = np.arccos(ca / c / a) / np.pi * 180.0
        gamma = np.arccos(ab / a / b) / np.pi * 180.0
            ...
            gridfile.write(
                textwrap.dedent("""\
                    CELL_PARAMETERS  {:10.3f} {:10.3f} {:10.3f}
                        CELL_ANGLES  {:10.3f} {:10.3f} {:10.3f}
                       GRID_NUMBERS        {}       {}       {}"""
                    .format(a, b, c, alpha, beta, gamma, *self.shape[:-1])
                )
            )
        # Write times file.
...
if __name__ == "__main__":
    ...
    egrid_dataset = EnergyShapeDataset(
                        #path="/home/INPUT_MESGAN/ZEOLITE/INPUT/half-zeo-3c",
                        path="test/test-dataset",
                        shape=32,
                        invert=True,
                        rotate=True,
                        move=True,
